...
var readChar = function readChar(reader) {
  ...
  isNewline(peekChar(reader)) ?
    (function() {
      reader["line"] = inc(reader["line"]);
      return reader["column"] = -1;
    })() :
    reader["column"] = inc(reader["column"]);
  return ch;
};
...
var unreadChar = function unreadChar(reader, ch) {
  return isNewline(ch) ?
    (function() {
      reader["line"] = dec(reader["line"]);
      return reader["column"] = count(reader["lines"][reader["line"]]);
    })() :
    reader["column"] = dec(reader["column"]);
};
...
var readerError = function readerError(reader, message) {
  ...
  var error = SyntaxError(text, reader["uri"]);
  error.line = reader["line"];
  error.column = reader["column"];
...
var readToken = function readToken(reader, initch) {
  return (function loop(buffer, ch) {
    ...
    while (recur === loop) {
      ...
      buffer :
      (buffer = str(buffer, readChar(reader)), ch = peekChar(reader), loop);
    };
    return recur;
