...
function expandRecur(form, names) {
  return form.map(function(form) {
    if (!_.isArray(form))
      ...
      return [ 'js-statement' ].concat(expressions).concat('loop');
    } else
      return expandRecur(form, names);
  });
}

keywords['js-statement'] = function(form) {
  if (form.length < 2) throw handleError(0, form._line, form._filename);
  return '(' + handleSubExpressions((form.slice(1))).join(', ') + ')';
};
keywords.loop = function(form) {
  if (form.length < 2) throw handleError(0, form._line, form._filename);
  var bindings = form[1];
  var names = bindings.filter(function(_, index) { return !(index % 2); });
  var values = bindings.filter(function(_, index) { return !!(index % 2); });


  var body = form.slice(2);
  return templates.loop({
    names: names.map(handleSymbol).join(', '),
    values: values.map(handleExpression).join(', '),
    body: handleExpressions(expandRecur(body, names)).replace(/^\s*return\s*/, '')
  });
};
