...
var parse = function(code, filename) {
    ...
    var parser = function() {
        ...
        while (pos < length) {
            ...
            if (c == '"') {
                // Continue reading string, if quote is escaped.
...
var handleExpression = function(expr) {
    ...
    // equivalent string.
    map(function(symbol) {
      return symbol[0] === "\\" && symbol.length === 2 ?
        "\"" + (symbol[1] === "\\" ? symbol : symbol.substr(1)) + "\"" :
        symbol;
    }).
...
function ParseError(message, line, uri) {
...
var handleError = function(no, line, filename) {
    return new ParseError(errors[no], line, filename);
};
...
keywords["re-pattern"] = function(arr) {
  var name = keywords["symbol*"](arr);
