...
_LS.statements = function(stmnts, isFunction) {
    ...
    var l = stmnts.length;
    for (var i = 0; i < l; i++) {
        var tmp = _LS.statementToString(stmnts[i]);
        if (tmp) {
            if (isFunction)
                if (i === l - 1)
                    tmp = "return " + tmp;
...
_LS.try = function(arr) {
    if (arr.length < 3) throw _LS.error(0, arr._line);
    var c = arr.pop();
    var ret = "(function () {try {" + _LS.statements(arr.slice(1), true) + 
              "} catch (err) {(" + _LS.statementToString(c) + ")(err)}})()"
    return ret;
}
...
this._compile = function(code) {
  ...
  return _LS.gen + _LS.statements(tree); 
}
