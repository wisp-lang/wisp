...
var macroExpand = function(tree) {
    var command = tree[0],
        ...
        method = macro[length] || macro.variadic,
...
keywords["macro"] = function(arr) {
    if (arr.length < 3) throw handleError(0, arr._line, arr._filename);
    var name = arr[1];
    if (!_.isArray(arr[2][0]))
      return keywords.macro([ "macro", name, arr.slice(2) ]);
    var methods = arr.slice(2);
    var macro = macros[name] = {};

    methods.forEach(function(method) {
      var params = method[0];
      var rules = method[1];
      var isVariadic = params.length &&
                       params[params.length - 1].indexOf("...") > 0;
      macro[isVariadic ? "variadic" : params.length] = {
        template: params,
        code: rules
      };
    });
