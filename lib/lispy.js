...
exports.run = function() {
  return (process.argv.length === 2) ?
    ...
    (process.argv.length === 3) ?
      compile((fs).createReadStream(process.argv[2]), process.stdout, (path).resolve(process.argv[2])) :
      compile((fs).createReadStream(process.argv[2]), (fs).createWriteStream(process.argv[3]), (path).resolve(process.argv[2]));
};
